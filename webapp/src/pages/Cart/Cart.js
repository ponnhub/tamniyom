...
const Cart = () => {
  ...
  useEffect(() => {
    ...
    async function getItems() {
      await db.getCart(customer_id).then(products => {
  ...
  const productsCard = (obj) => {
    ...
    <CardMedia
        ...
        to={`/product/${product.catid}/${product.productid}`}
  ...
  return (
    <Page pageTitle={intl.formatMessage({ id: 'Cart' })}>
      ...
      <Stack spacing={2}>
        ...
        <Button sx={{ width: '50%' }} variant="contained" disabled={!products.length}>ยืนยันส่งคำสั่งซื้อ</Button>
      </Stack>      
    </Box>
    </Page>
  )
